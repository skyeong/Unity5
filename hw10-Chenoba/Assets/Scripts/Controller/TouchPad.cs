...
public class TouchPad : MonoBehaviour {
	...
	public float distanceAway = 1.2f;
	...
	void HandleInput (Vector3 input)
	{
		...
		//Camera Movement !!
		Vector3 oldPos = CameraFollower.position;
		Vector3 newPos = new Vector3 (
			Mathf.Clamp(oldPos.x + (normDiff.x * distanceAway), -14f, 14f), 
			Mathf.Clamp(oldPos.y + (normDiff.y * distanceUp), 1f, 20f ), 
			oldPos.z
		);
		CameraFollower.position = newPos;
		//Debug.Log (newPos);
	}
}
